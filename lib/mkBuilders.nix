...
with nixpkgs.lib;
  {self, ...} @ inputs: let
    # remove self to break infinite recursion
    ...
    nixosActivations = nodes:
      builtins.mapAttrs nixosActivation (
        attrsets.filterAttrs (
          systemName: systemConfig:
            if (systemConfig ? "imageBuild")
            then !systemConfig.imageBuild
            else true
        )
        nodes
      );
  ...
  in {
    ...
    devshellImport = import ./mkDevshellImport.nix nixpkgs deploy-rs inputsMinusSelf;
